...
export const initialFormData: PayrollFormData = {
    ...
    caFilingStatus: 'single', caTaxCredit: 0, caEstimatedDeductions: 0,
    ...
    ohMunicipality: '', paExemptStateTax: false, paResidencyMunicipality: '', paWorkplaceMunicipality: '',
...
const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label: string, error?: string, info?: string }> = ({ label, error, info, ...props }) => (
    <div className="w-full">
        <div className="flex items-center gap-1.5 mb-1">
            <label htmlFor={props.id} className="block text-sm font-medium text-gray-700">{label}</label>
            {info && (
                <div className="group relative flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-400 hover:text-gray-600 cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    <div className="absolute bottom-full mb-2 w-64 bg-gray-800 text-white text-xs rounded py-2 px-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10 left-1/2 -translate-x-1/2">
                        {info}
...
const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement> & { label: string, error?: string, info?: string }> = ({ label, error, children, info, ...props }) => (
     <div>
        <div className="flex items-center gap-1.5 mb-1">
            <label htmlFor={props.id} className="block text-sm font-medium text-gray-700">{label}</label>
            {info && (
...
const Checkbox: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label: string, info?: string }> = ({ label, info, ...props }) => (
...
const StateSpecificFields = React.memo(({ data, handleChange, errors, isContractor }: { data: PayrollFormData, handleChange: (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => void, errors: Record<string, string>, isContractor: boolean }) => {
    ...
    switch (data.state) {
        ...
        case 'CA': return (<><StateInfoBox>Under TY 2026 OBBB rules, California uses a simplified filing status and a State Tax Credit to determine withholding, replacing the previous allowance system.</StateInfoBox><Select label="CA State Filing Status" id="caFilingStatus" name="caFilingStatus" value={data.caFilingStatus} onChange={handleChange} disabled={isContractor}><option value="single">Single</option><option value="married">Married</option><option value="head_of_household">Head of Household</option></Select><Input label="CA State Tax Credit ($ per period)" id="caTaxCredit" name="caTaxCredit" type="number" min="0" step="0.01" value={data.caTaxCredit} onChange={handleChange} disabled={isContractor} info="From Form DE 4. A dollar-for-dollar reduction of calculated California state income tax for this pay period. This replaces the old allowance system." /><Input label="CA Estimated Deductions ($)" id="caEstimatedDeductions" name="caEstimatedDeductions" type="number" min="0" step="0.01" value={data.caEstimatedDeductions} onChange={handleChange} disabled={isContractor} error={errors.caEstimatedDeductions} /><Input label="CA Est. Non-Wage Income ($)" id="caEstimatedNonWageIncome" name="caEstimatedNonWageIncome" type="number" min="0" step="0.01" value={data.caEstimatedNonWageIncome} onChange={handleChange} disabled={isContractor} error={errors.caEstimatedNonWageIncome} /><Input label="CA Additional Withholding ($)" id="caAdditionalWithholding" name="caAdditionalWithholding" type="number" min="0" step="0.01" value={data.caAdditionalWithholding} onChange={handleChange} disabled={isContractor} /><div className="md:col-span-2 space-y-3 pt-2"><h4 className="font-semibold text-gray-600 text-sm">CA Tax Exemptions</h4><Checkbox label="Exempt from State Income Tax" id="caExemptStateTax" name="caExemptStateTax" checked={data.caExemptStateTax} onChange={handleChange} disabled={isContractor} /><Checkbox label="Exempt from State Disability (SDI)" id="caExemptSdi" name="caExemptSdi" checked={data.caExemptSdi} onChange={handleChange} disabled={isContractor} info="State Disability Insurance (SDI) provides short-term wage replacement benefits to eligible California workers. Most employees contribute." /></div></>);
        ...
        case 'CT': return (<><StateInfoBox>Connecticut uses Form CT-W4, which specifies a withholding code based on filing status.</StateInfoBox><Select label="CT Withholding Code" id="ctWithholdingCode" name="ctWithholdingCode" value={data.ctWithholdingCode} onChange={handleChange} disabled={isContractor} info="Found on Form CT-W4. This code determines the tax withholding table to use based on your specific filing status and situation."><option value="A">A (Single/Married Separate, higher rate)</option><option value="B">B (Head of Household)</option><option value="C">C (Married/Civil Union Joint)</option><option value="D">D (Single/Married Separate, lower rate)</option><option value="F">F (Married/Civil Union Joint, one spouse works)</option></Select><div className="md:col-span-1"></div><div className="md:col-span-2 space-y-3 pt-2"><h4 className="font-semibold text-gray-600 text-sm">CT Tax Exemptions</h4><Checkbox label="Exempt from State Income Tax" id="ctExemptStateTax" name="ctExemptStateTax" checked={data.ctExemptStateTax} onChange={handleChange} disabled={isContractor} /></div></>);
        ...
        case 'NJ': return (<><StateInfoBox>Under TY 2026 OBBB rules, New Jersey uses a simplified filing status and a State Tax Credit to determine withholding, replacing the previous A-E filing rates and allowance system.</StateInfoBox><Select label="NJ State Filing Status" id="njFilingStatus" name="njFilingStatus" value={data.njFilingStatus} onChange={handleChange} disabled={isContractor}><option value="single">Single</option><option value="married">Married</option><option value="head_of_household">Head of Household</option></Select><Input label="NJ State Tax Credit ($ per period)" id="njTaxCredit" name="njTaxCredit" type="number" min="0" value={data.njTaxCredit} onChange={handleChange} disabled={isContractor} info="From Form NJ-W4. A dollar-for-dollar reduction of calculated New Jersey state income tax for this pay period. Replaces the previous A-E filing rates." /><div className="md:col-span-2 space-y-3 pt-2"><h4 className="font-semibold text-gray-600 text-sm">NJ Tax Exemptions</h4><Checkbox label="Exempt from State Income Tax" id="njExemptStateTax" name="njExemptStateTax" checked={data.njExemptStateTax} onChange={handleChange} disabled={isContractor} /><Checkbox label="Exempt from SUI/SDI" id="njExemptSuiSdi" name="njExemptSuiSdi" checked={data.njExemptSuiSdi} onChange={handleChange} disabled={isContractor} info="State Unemployment & Disability Insurance. Exemption is rare, e.g., for some family employment." /><Checkbox label="Exempt from Family Leave Insurance (FLI)" id="njExemptFli" name="njExemptFli" checked={data.njExemptFli} onChange={handleChange} disabled={isContractor} info="Family Leave Insurance provides paid time off for family care. Exemption is very rare for employees." /></div></>);
        ...
        case 'NY': return (<><StateInfoBox>New York withholding includes state tax (Form IT-2104) and potentially local tax for cities like NYC and Yonkers.</StateInfoBox><Select label="NY State Filing Status" id="nyStateFilingStatus" name="nyStateFilingStatus" value={data.nyStateFilingStatus} onChange={handleChange} disabled={isContractor}><option value="single">Single</option><option value="married">Married</option><option value="head_of_household">Head of Household</option></Select><Input label="NY State Allowances" id="nyStateAllowances" name="nyStateAllowances" type="number" min="0" value={data.nyStateAllowances} onChange={handleChange} disabled={isContractor} /><Input label="NY Additional Withholding ($)" id="nyAdditionalWithholding" name="nyAdditionalWithholding" type="number" min="0" step="0.01" value={data.nyAdditionalWithholding} onChange={handleChange} disabled={isContractor} error={errors.nyAdditionalWithholding} info="An extra flat-dollar amount you want withheld from each paycheck for NY state tax, as specified on Form IT-2104." /><Select label="NY Work City (for local tax)" id="nyWorkCity" name="nyWorkCity" value={data.nyWorkCity} onChange={handleChange} disabled={isContractor}><option value="none">Outside NYC / Yonkers</option><option value="nyc">New York City</option><option value="yonkers">Yonkers</option></Select><div className="md:col-span-2 space-y-3 pt-2"><h4 className="font-semibold text-gray-600 text-sm">NY Tax Exemptions</h4><Checkbox label="Exempt from State Income Tax" id="nyExemptStateTax" name="nyExemptStateTax" checked={data.nyExemptStateTax} onChange={handleChange} disabled={isContractor} /><Checkbox label="Exempt from State Disability (NYSDI)" id="nyExemptSdi" name="nyExemptSdi" checked={data.nyExemptSdi} onChange={handleChange} disabled={isContractor} info="New York State Disability Insurance provides short-term wage replacement. Most employees contribute." /><Checkbox label="Employee has a waiver for Paid Family Leave (PFL)" id="nyPflWaiver" name="nyPflWaiver" checked={data.nyPflWaiver} onChange={handleChange} disabled={isContractor} info="Paid Family Leave (PFL) provides paid time off for family care. Employees can only apply for a waiver under very specific circumstances, such as not working long enough to be eligible." /></div></>);
        ...
        case 'PA': return (<><StateInfoBox>Under TY 2026 OBBB rules, PA local taxes (EIT/LST) are determined by municipality names instead of PSD codes to simplify input.</StateInfoBox><Input label="Residency Municipality" id="paResidencyMunicipality" name="paResidencyMunicipality" type="text" placeholder="e.g., Upper Darby Township" value={data.paResidencyMunicipality} onChange={handleChange} disabled={isContractor} info="The municipality where the employee lives. Used to calculate local Earned Income Tax (EIT)." /><Input label="Workplace Municipality" id="paWorkplaceMunicipality" name="paWorkplaceMunicipality" type="text" placeholder="e.g., Philadelphia" value={data.paWorkplaceMunicipality} onChange={handleChange} disabled={isContractor} info="The municipality where the employee works. The higher of the residency and workplace EIT rates is used for withholding." /><div className="md:col-span-2 space-y-3 pt-2"><h4 className="font-semibold text-gray-600 text-sm">PA Tax Exemptions</h4><Checkbox label="Exempt from State Income Tax" id="paExemptStateTax" name="paExemptStateTax" checked={data.paExemptStateTax} onChange={handleChange} disabled={isContractor} /><Checkbox label="Exempt from Local Services Tax (LST)" id="paIsExemptLST" name="paIsExemptLST" checked={data.paIsExemptLST} onChange={handleChange} disabled={isContractor} info="The Local Services Tax (LST) is a flat-rate local tax, typically withheld from employee paychecks. Certain individuals with low income may be exempt." /></div></>);
        case 'RI': return (<><StateInfoBox>Rhode Island uses Form RI W-4. Withholding is based on allowances. RI also has a mandatory Temporary Disability Insurance (TDI) contribution.</StateInfoBox><Input label="RI Allowances" id="riAllowances" name="riAllowances" type="number" min="0" value={data.riAllowances} onChange={handleChange} disabled={isContractor} /><Input label="RI Additional Withholding ($)" id="riAdditionalWithholding" name="riAdditionalWithholding" type="number" min="0" step="0.01" value={data.riAdditionalWithholding} onChange={handleChange} disabled={isContractor} /><div className="md:col-span-2 space-y-3 pt-2"><h4 className="font-semibold text-gray-600 text-sm">RI Tax Exemptions</h4><Checkbox label="Exempt from State Income Tax" id="riExemptStateTax" name="riExemptStateTax" checked={data.riExemptStateTax} onChange={handleChange} disabled={isContractor} /><Checkbox label="Exempt from Temporary Disability Insurance (TDI)" id="riExemptTdi" name="riExemptTdi" checked={data.riExemptTdi} onChange={handleChange} disabled={isContractor} info="Temporary Disability Insurance (TDI) provides temporary income for workers who are unable to work due to a non-work-related illness or injury. Most employees contribute." /></div></>);
...
export function PayrollForm({ data, onDataChange, onSubmit, suggestedTaxes, onSuggestionsChange }: PayrollFormProps) {
    ...
    const getOvertimeInfo = useCallback((state: PayrollFormData['state']) => {
        switch (state) {
            case 'CA':
            case 'AK':
            case 'NV':
            case 'CO':
                return {
                    multiplier: 1.5,
                    info: `The standard rate is 1.5x. ${state} also has daily overtime laws (e.g., after 8 hours/day), and in some cases double-time, which are not automatically calculated. Please adjust manually if daily OT applies.`
                };
            case 'OR':
                return {
                    multiplier: 1.5,
                    info: `The standard rate is 1.5x for hours over 40/week. Some industries in Oregon, like manufacturing, have daily overtime rules.`
                };
            case 'KY':
                 return {
                    multiplier: 1.5,
                    info: `The standard rate is 1.5x for hours over 40/week. Kentucky law also requires overtime pay for work on the 7th consecutive day of the workweek.`
                 };
            default:
                return {
                    multiplier: 1.5,
                    info: "The standard overtime multiplier is 1.5x, based on federal (FLSA) and state guidelines for hours worked over 40 in a week. This rate is automatically selected."
                };
        }
    }, []);

    const overtimeInfoText = useMemo(() => getOvertimeInfo(data.state).info, [data.state, getOvertimeInfo]);
    ...
    const handleChange = useCallback((e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        onDataChange(prevData => {
            ...
            if (name === 'overtimeHoursWorked') {
                // Re-apply the state-specific multiplier whenever OT hours are adjusted.
                const { multiplier } = getOvertimeInfo(prevData.state);
                newData.overtimeRateMultiplier = multiplier;
            }
            return newData;
        });
    }, [suggestedTaxes, onDataChange, onSuggestionsChange, getOvertimeInfo]);

    const resetStateFields = (resetState: PayrollFormData) => ({
        njFilingStatus: resetState.njFilingStatus, njTaxCredit: resetState.njTaxCredit, njExemptSuiSdi: resetState.njExemptSuiSdi,
        ...
        caFilingStatus: resetState.caFilingStatus, caTaxCredit: resetState.caTaxCredit, caEstimatedDeductions: resetState.caEstimatedDeductions,
        ...
        paExemptStateTax: resetState.paExemptStateTax, paResidencyMunicipality: resetState.paResidencyMunicipality, paWorkplaceMunicipality: resetState.paWorkplaceMunicipality,
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 md:p-8 rounded-lg shadow-lg">
            ...
            <Section title="Tax Withholding (TY 2026 OBBB Rules)" info="Under OBBB, allowances are replaced with tax credits, which directly reduce calculated tax. Standard deductions are still based on filing status.">
                ...
                <Input label="Federal Tax Credit ($ per period)" id="federalTaxCredit" name="federalTaxCredit" type="number" min="0" step="0.01" value={data.federalTaxCredit} onChange={handleChange} disabled={isContractor} info="From Form W-4, Step 3. A dollar-for-dollar reduction of calculated federal income tax for this pay period. This replaces the old allowance system under OBBB rules." />
            ...
            {data.employeeType === 'employee' && (
                <Section title="Employer Taxes" info="These are state-level taxes paid by the employer, not deducted from the employee's pay. The most common is State Unemployment Tax (SUTA).">
                    <Input label="State Unemployment (SUTA) Rate (%)" id="employerSutaRate" name="employerSutaRate" type="number" min="0" step="0.001" value={data.employerSutaRate} onChange={handleChange} info="The State Unemployment Tax Act (SUTA) rate is assigned to the employer by the state. This is an employer-paid tax and does not affect employee net pay." />
                </Section>
            )}
